...
public class WorkingInfo {
	...
	public synchronized void incrementJob(JobExecutionContext jobExecutionContext) {
		...
		String system = jobExecutionContext.getUsePlatform() ? Symbol.XING_HAO : jobExecutionContext.getSystem();
		Integer jobNum = systemMap.get(system);
		if (jobNum == null) {
			systemMap.put(system, 1);
		} else {
			systemMap.put(system, jobNum + 1);
		}
	...
	public synchronized void decrementJob(JobExecutionContext jobExecutionContext) {
		...
		if (systemMap != null) {
			String system = jobExecutionContext.getUsePlatform() ? Symbol.XING_HAO : jobExecutionContext.getSystem();
			Integer jobNum = systemMap.get(system);
			systemMap.put(system, jobNum = jobNum - 1 < 0 ? 0 : jobNum - 1);
		}
