...
public class IceKey {
	...
	private static final int sMod[][] = { { 0x33, 0x31, 0x50, 0x36 }, { 0x37, 0x37, 0x31, 0x39 },
			{ 0x31, 0x45, 0x51, 0x37 }, { 0x39, 0x42, 0x39, 0x50 } };
	...
	private static class IceKeyIner {
		private static IceKey instance = new IceKey(2);
	}
		...
		while (true) {
			for (int j = 0; j < 8; j++) {
				if (n >= kd.length) {
					...
					ciphertext[j] = getHexCode(sData.charAt(n), sData.charAt(n + 1));
					n += 2;
				}
			}
	...
	public static String getEncode(String sData, String sKey) {
		return IceKeyIner.instance.encode(sData, sKey);
	}

	public static String getDecode(String sData, String sKey) {
		return IceKeyIner.instance.decode(sData, sKey);
	}

	public static void main(String[] args) {
		String sKey = "user";
		String sData = "dddd";
		String data = getEncode(sData, sKey);
		System.out.println(data);
		data = getDecode(data, sKey);
		System.out.println(data);
	}
