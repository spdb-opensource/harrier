...
public interface MAlarmSendQueueMapper {
	...
	@ResultMap("MAlarmSendQueueResult")
	@SelectProvider(type = SqlProviderAdapter.class, method = "select")
	List<MAlarmSendQueue> selectManyPage(SelectStatementProvider selectStatement, Page<MAlarmSendQueue> page);

	default Page<MAlarmSendQueue> search(Page<MAlarmSendQueue> page){
		SelectStatementProvider selectStatement = SqlBuilder.select(selectList).from(MAlarmSendQueue)
				.build()
				.render(RenderingStrategies.MYBATIS3);
		List<MAlarmSendQueue> records = selectManyPage(selectStatement, page);
