...
public interface UdsJobMenuMapper{
	 ...
	 */
	BasicColumn[] selectList = BasicColumn.columnList(UdsJobDynamicSqlSupport.id,UdsJobDynamicSqlSupport.platform, UdsJobDynamicSqlSupport.systems, UdsJobDynamicSqlSupport.job, lastStatus, jobDate, nextJobDate,
			serverName, multiBatch, pendingTime, dispatcherTime, startTime, endTime, numTimes, callAgainNum, streamType,
			UdsJobDynamicSqlSupport.des, jobName,jobType,offsetDay,timeWindow,virtualEnable,priority,callAgainMaxNum,callAgainWaitSec,countBatch,batchConversion,checkFrequency,checkTimeTrigger
	...
	default Page<UdsJobMenu> selectAll(Page<UdsJobMenu> page,String platform_,String systems_,String job_,String last_status_) {
		SelectStatementProvider selectStatement = SqlBuilder.select(selectList)
				.from(UdsJobDynamicSqlSupport.udsJob)
				...
				.where(udsJob.job,isLikeWhenPresent(job_))
				.and(udsJob.platform,isEqualTo(platform_))
				.and(udsJob.systems,isEqualToWhenPresent(systems_))
				.and(udsJob.lastStatus,isEqualToWhenPresent(last_status_))
				.build()
	...
	default UdsJobMenu selectJobDetail(String platform_,String systems_,String job_) {
		SelectStatementProvider selectStatement = SqlBuilder.select(selectList)
				...
				.and(systems, equalTo(UdsJobConfigDynamicSqlSupport.systems))
				.and(job, equalTo(UdsJobConfigDynamicSqlSupport.job))
				.where(udsJob.job,isEqualTo(job_))
				.and(udsJob.platform, isEqualTo(platform_))
				.and(udsJob.systems, isEqualTo(systems_))
				.build()
				.render(RenderingStrategies.MYBATIS3);
	...
	default int updateJobDetail(UdsJobMenu udsJobMenu) {
		UpdateStatementProvider updateStatement1 = SqlBuilder.update(udsJob)
				.set(jobDate).equalToWhenPresent(udsJobMenu.getJobDate())
				.set(lastStatus).equalToWhenPresent(udsJobMenu.getLastStatus())
				...
				.and(udsJob.systems, isEqualTo(udsJobMenu.getSystems()))
				.build()
				.render(RenderingStrategies.MYBATIS3);
		
		UpdateStatementProvider updateStatement2 = SqlBuilder.update(udsJobConfig)
				.set(priority).equalToWhenPresent(udsJobMenu.getPriority())
				.set(callAgainMaxNum).equalToWhenPresent(udsJobMenu.getCallAgainMaxNum())
				.set(checkFrequency).equalToWhenPresent(udsJobMenu.getCheckFrequency())
				.set(checkTimeTrigger).equalToWhenPresent(udsJobMenu.getCheckTimeTrigger())
				.set(timeWindow).equalToWhenPresent(udsJobMenu.getTimeWindow())
				.set(ignoreError).equalToWhenPresent(udsJobMenu.getIgnoreError())
				.where(udsJob.job,isEqualTo(udsJobMenu.getJob()))
				.and(udsJob.platform, isEqualTo(udsJobMenu.getPlatform()))
				.and(udsJob.systems, isEqualTo(udsJobMenu.getSystems()))
				.build()
	...
	default Page<UdsJobMenu> listUpJobs(Page<UdsJobMenu> page, String job_) {
		...
		SelectStatementProvider selectStatement = SqlBuilder.select(constant("t1.platform,t1.systems,t2.job pjob,t1.job,t1.server_name,t3.job_type,t1.last_status,t2.dep_batch depBatch,t1.start_time,t1.end_time,t1.job_date,t2.is_enable depIsEnable,t2.des,t2.id pid"))
				.from(t1,"t1")
				.leftJoin(t2,"t2")
				.on(t1.platform,equalTo(t2.depPlatform))
				.and(t1.systems,equalTo(t2.depSystem))
				.and(t1.job,equalTo(t2.depJob))
				.leftJoin(t3,"t3")
				.on(t1.platform,equalTo(t3.platform))
				.and(t1.systems,equalTo(t3.systems))
				.and(t1.job,equalTo(t3.job))
				.where(t2.job,isEqualTo(job_))
				.build()
				.render(RenderingStrategies.MYBATIS3);
	...
	default List<UdsJobMenu> listAllUpJobs(String[] job_) {
		...
		SelectStatementProvider selectStatement = SqlBuilder.select(constant("t1.platform,t1.systems,t2.job pjob,t1.job,t1.server_name,t3.job_type,t1.last_status,t1.pending_time,t1.dispatcher_time,t1.start_time,t1.end_time,t1.job_date,t1.multi_batch,t1.num_times,t3.priority,t3.time_window,t3.is_enable,t2.is_enable depIsEnable,t3.check_time_trigger"))
				.from(t1,"t1")
				.leftJoin(t2,"t2")
				.on(t1.platform,equalTo(t2.depPlatform))
				.and(t1.systems,equalTo(t2.depSystem))
	...
	default List<UdsJobMenu> listAllDownjobs(String[] job_) {
		...
		SelectStatementProvider selectStatement = SqlBuilder.select(constant("t1.platform,t1.systems,t2.dep_job pjob,t1.job,t1.server_name,t3.job_type,t1.last_status,t1.pending_time,t1.dispatcher_time,t1.start_time,t1.end_time,t1.job_date,t1.multi_batch,t1.num_times,t3.priority,t3.time_window,t3.is_enable,t2.is_enable depIsEnable,t3.check_time_trigger"))
				.from(t1,"t1")
				.leftJoin(t2,"t2")
				.on(t1.platform,equalTo(t2.platform))
				.and(t1.systems,equalTo(t2.systems))
				.and(t1.job,equalTo(t2.job))
				.leftJoin(t3,"t3")
				.on(t1.platform,equalTo(t3.platform))
				.and(t1.systems,equalTo(t3.systems))
				.and(t1.job,equalTo(t3.job))
				.where(t2.depJob,isEqualTo(job_[0]))
				.build()
				.render(RenderingStrategies.MYBATIS3);
		System.out.print(1);
		List<UdsJobMenu> records= selectMany(selectStatement);
