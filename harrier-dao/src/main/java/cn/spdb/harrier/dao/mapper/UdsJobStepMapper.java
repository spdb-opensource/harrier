...
public interface UdsJobStepMapper {
    ...
    BasicColumn[] selectList = BasicColumn.columnList(id, platform, systems, job, stepType, stepNum, operCmd, scriptPath, parameter,updateTime, environments, workDir, isEnable, des);
    ...
    default int insertMultiple(Collection<UdsJobStep> records) {
        return MyBatis3Utils.insertMultiple(this::insertMultiple, records, udsJobStep, c ->
            c.map(id).toProperty("id")
            .map(platform).toProperty("platform")
            .map(systems).toProperty("systems")
            .map(job).toProperty("job")
    ...
    default int insertSelective(UdsJobStep record) {
        return MyBatis3Utils.insert(this::insert, record, udsJobStep, c ->
            ...
            .map(job).toPropertyWhenPresent("updateTime", record::getUpdateTime)
            .map(updateTime).toPropertyWhenPresent("job", record::getJob)
            .map(stepType).toPropertyWhenPresent("stepType", record::getStepType)
            .map(stepNum).toPropertyWhenPresent("stepNum", record::getStepNum)
            .map(operCmd).toPropertyWhenPresent("operCmd", record::getOperCmd)
    ...
    static UpdateDSL<UpdateModel> updateAllColumns(UdsJobStep record, UpdateDSL<UpdateModel> dsl) {
        return dsl.set(id).equalTo(record::getId)
                ...
                .set(stepType).equalTo(record::getStepType)
                .set(stepNum).equalTo(record::getStepNum)
                .set(operCmd).equalTo(record::getOperCmd)
    ...
    static UpdateDSL<UpdateModel> updateSelectiveColumns(UdsJobStep record, UpdateDSL<UpdateModel> dsl) {
        return dsl.set(id).equalToWhenPresent(record::getId)
                ...
                .set(stepType).equalToWhenPresent(record::getStepType)
                .set(stepNum).equalToWhenPresent(record::getStepNum)
                .set(operCmd).equalToWhenPresent(record::getOperCmd)
