...
public interface UdsJobWeightMapper {
    ...
    BasicColumn[] selectList = BasicColumn.columnList(id, platform, systems, job, limitType, confValue, desc);
	...
	@ResultMap("UdsJobWeightResult")
	@SelectProvider(type = SqlProviderAdapter.class, method = "select")
	List<UdsJobWeight> selectManyPage(Page<UdsJobWeight> page,SelectStatementProvider selectStatement);
	default Page<UdsJobWeight> selectAll(Page<UdsJobWeight> page,String platform_,String systems_,String job_){
		...
		List<UdsJobWeight> records = selectManyPage(page,selectStatement);
