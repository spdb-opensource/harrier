 ...
 *
 */
public class AppointOrderDispatcher extends AbstractDispatcherPlan {
	...
	public UdsRpcClient getUdsRpcClient(UdsJobBean udsJobBean, UdsSystemBean udsSystemBean, boolean over) {
		...
		if (StringUtils.isBlank(str)) {
			...
			for (Entry<String, ChildServerInfo> entry : MasterManager.getInstance().getChildServerJobMap().entrySet()) {
				...
				UdsRpcClient client = UdsRpcClientManager.getInstance().getUdsRpcClient(date.getName());
				if (client == null) {
					continue;
				}
				String order = String.valueOf(client.getOrder());
