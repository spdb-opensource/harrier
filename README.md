...
harrier海鹞为Data Ocean品牌体系下的调度工具类产品，由Java开发，支持Linux,Windows,AIX环境部署，支持各类异构数据计算平台的作业调度（Hive,Spark,Teradata,Oracke,DB2,DataStage等），支持各类自定义任务作业调度（Java，Shell，Pyth等）。

------

### 项目特点

1.Harrier与现有的互联网调度产品不同，现有的调度产品均基于DAG图进行工作流管理，Harrier简化了作业流管理概念。

2.Harrier在作业层之上抽象出了计算平台、应用的概念，实现了单一调度系统覆盖全企业视图的数据处理平台，并支持丰富的权限管理功能，任务管理和监控功能。
