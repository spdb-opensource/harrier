<template>
	<div>
		<div v-if="tabObj.alarmTab">
			...
			<div id="alarmList" class="spdb-table" style="margin-top:5px;">
      ...
      <Col span="24">
				<div class='marquee-wrap' @mouseenter="enter" @mouseout="leave">
					<transition-group name='list' tag='p'>
						<span v-for="(item, index) in showData" :key="index" class="list-item duty">
							<span class='dutys'>值班信息 [</span>{{item}}<span class='dutys'>]</span>
						</span>
			...
			<!-- <Col span="2">
				<div style="text-align:right;margin-right:10px;" class='duty'>
					<a href="#" slot="extra" @click.prevent="tomain" >
		...
		<Modal
            ...
            <Row>
						...
						<Row>
							<FormItem  style='width:75%' prop="sendParams" label="发送参数">
								<Input readonly v-model="cuInfoFormBean.sendParams"/>
							</FormItem>
						</Row>
					</Form>
				</div>
		...
		<Modal
            ...
            <Row>
							<FormItem  style='width:75%' prop="operationType" label="处理类型">
								<!-- <Select filterable  v-model="formBean.operationType" >
									<Option v-for="item in operationTypeList" :value="item.value" :key="item.value">{{ item.label }}</Option>
								</Select> -->
                <Input v-model="formBean.operationType"/>
							</FormItem>
						</Row>
						<Row>
							<FormItem style='width:75%' prop="operationUser" label="处理人">
								<Input v-model="formBean.operationUser"/>
							</FormItem>
						</Row>

						<Row>
							<FormItem  style='width:75%' prop="operationTime" label="处理时间">
								<Input readonly v-model="formBean.operationTime"/>
							</FormItem>
						</Row>
						<Row>
							<FormItem style='width:75%' ref="alarmStatusSelect" prop="alarmStatus" label="告警状态">
								<Select filterable  v-model="formBean.alarmStatus" >
									<Option v-for="item in alarmstatusList" :value="item.value" :key="item.value">{{ item.label }}</Option>
								</Select>
							</FormItem>
						</Row>
...
export default {
  ...
  data: function () {
    ...
    return {
      ...
      formBean: {},
      tabObj: { alarmTab: true, jobdetail: false },
      request: {
        delReq: {
          method: 'DELETE',
          url: RESOURCE_PATH
      ...
      gridColumns: [
              ...
              h('a', {
                ...
                on: {
                  click: () => {
                    ...
                    this.$emit('switch', Object.assign({}, { id: id, statusName: 'jobdetail', alarmTab: false }, this.getPageParam()))
                  }
                }
              }, row.job)
            ])
        ...
        },
        {
          ...
          width: 120,
          render: (h, { column, index, row }) => {
            // let staName = row.alarmStatus == '2' ? '恢复告警' : (row.alarmStatus == '0'? '暂停告警':'已处理');
            // let staName = row.alarmStatus
            return h('div', [
              // h('a', {
              //   props: {
              //     size: 'small',
  ...
  methods: {
    ...
    errorHandleOk () {
      this.$refs.alarmHandleForm.validate(valid => {
        ...
        //   params.authps = this.formBean.job.split('_')[0] + this.formBean.job.split('_')[1]
        // } else {
    ...
    errorHandleBack () {
      this.$refs.alarmStatusSelect.classes[1]['ivu-form-item-error'] = false
      if (this.$refs.alarmStatusSelect.$el.children[1].children[1]) {
        this.$refs.alarmStatusSelect.$el.children[1].children[1].remove()
      }
      this.errorHandle.show = false
    },
    showErrorHandle (errorObj, index) {
      this.formBean = {}
      // this.formBean.index = index
      this.errorHandle.show = true
      this.formBean.id = errorObj.id
      // this.formBean.operation = errorObj.operation == null ? '0' : errorObj.operation
      // this.formBean.alarmStatus = errorObj.alarmStatus
      // this.formBean.job = errorObj.job
      // this.formBean.platform = errorObj.platform
      // this.formBean.system = errorObj.system
    ...
    querySystem () {
      let loadConfig = {
        method: 'GET',
        url: '/msys/getSysList',
        params: { platform: this.alarmBean.platform }
      }
      this.systemData = []
