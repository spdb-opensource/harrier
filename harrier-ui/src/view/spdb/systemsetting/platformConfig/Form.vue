	...
	<div>
		<Form ref="systemForm" :model="formBean" :label-width="180" :rules="formRule">
			<Row>
				<Col span="5">
					<FormItem prop="platform" label="平台名">
						<Input v-model="formBean.platform" disabled/>
					</FormItem>
				</Col>
			</Row>
			<Row>
				<Col span="5">
					<FormItem prop="system" label="应用名">
						<Input v-model="formBean.system" disabled/>
					</FormItem>
				</Col>
			</Row>
			<Row>
				<Col span="5">
					<FormItem prop="des" label="描述">
						<Input v-model="formBean.des" disabled/>
					</FormItem>
				</Col>
			</Row>
			<!--
			...
			<Row>
				...
				<Col span="7">
					<FormItem prop="strategy" label="分发策略" >
						<Select filterable v-model="formBean.strategy">
			...
			<Row>
				<Col span="7" >
					<FormItem prop="strategyPro" label="策略参数">
						<Input v-model="formBean.strategyPro" placeholder="执行节点名称以,隔开"/>
    ...
    save () {
      this.$refs.systemForm.validate(valid => {
        ...
        if (this.transData.sys && this.transData.platform) {
          ...
          httpConfig.url = RESOURCE_PATH + '/' + this.transData.platform + '_' + this.transData.sys
        } else {
